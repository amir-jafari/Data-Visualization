...
| **`basics/`** | 64 one-idea-per-file lessons: text, charts, widgets, layout, state, caching. [Details →](basics/README.md) |
| **`apps/`** | 22 full apps that combine those pieces: NLP, vision, audio, ML, time series, maps. [Details →](apps/README.md) |
...
**1. Read a lesson, then change it.** Every file in `basics/` is small enough
...
See `basics/state_and_config/session_state.py` for the escape hatch
...
`basics/state_and_config/caching.py` shows both, and every app in
`apps/` uses them.

**4. Move to `apps/` when you want a project.** Each one is a `main.py` plus
a `utils.py`, and they need the extra install:

```bash
pip install -r streamlit/requirements-apps.txt
...
`streamlit/requirements.txt` covers the rest.

**`streamlit: command not found`** — your virtual environment isn't active, or
you installed into a different one. Use `python -m streamlit run streamlit/app.py`.

**Port already in use** — another app is still running. Pick another port:
`streamlit run streamlit/app.py --server.port 8888`.
